...
// LLM_GRAPH_TYPE_DECODER_MTP draft head for Nemotron-H MoE
llama_model_nemotron_h_moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    // token embedding weights (the MTP head ships its own tok_embd, shared with the trunk
    // when bundled or exported into the standalone MTP GGUF)
    ...
    // input fusion: eh_proj( [enorm(emb) ; hnorm(h)] )
    ...
    // MoE FFN sub-layer (mtp.layers.1): relu^2 experts + sigmoid group top-k, non-gated shared expert
    ...
    // final head norm: the MTP head's final_layernorm is a LayerNorm (not RMS)
    ...
    // LM head (the MTP head ships its own output projection, shared with the trunk when
    // bundled or exported into the standalone MTP GGUF)
